...
export default function SideBar({ unread_conversations_count } : IProps) {
    const router = useRouter();
    const [isOpen, setIsOpen] = useState<boolean>(false);
    console.log("in side bar", unread_conversations_count);
    // const router = useRouter();
            ...
            <nav className="flex justify-between items-center">
                ...
                <div className="flex gap-5 items-center">
                    ...
                    <div onClick={ () => { router.push('/conversations') }} className="relative cursor-pointer">
                        ...
                        { !!unread_conversations_count && <Badge className="bg-orange-400 rounded-full absolute -bottom-2 -right-3">{ unread_conversations_count }</Badge> }
                    </div>
